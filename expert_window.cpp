...
bool expert_window::wait_for_button()
// modified event loop:
...
void expert_window::cb_maroon(Address, Address pw)
{
	reference_to<expert_window>(pw).maroon();
}

void expert_window::cb_white(Address, Address pw)
{
	reference_to<expert_window>(pw).white();
}

void expert_window::cb_black(Address, Address pw)
{
...
void expert_window::maroon()
{
	if (observation_period == true) {
		...
		if (choices_left == 0) {
			...
			mdo::score_io fin_score{ "\121\WhenInDoubtCYourWayOut\GameGUI\GameGUI\expert.txt", 2560 };
